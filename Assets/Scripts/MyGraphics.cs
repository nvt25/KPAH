...
public class MyGraphics
{
	...
	private void cache(string key, Texture value)
	{
		...
		if (value.width * value.height < Canvas.w * Canvas.h)
		{
	...
	public void drawlineGL()
	{
		...
		for (int i = 0; i < totalLine.size(); i++)
		{
			...
			GL.Vertex(new Vector2(num, num2));
			GL.Vertex(new Vector2(num3, num4));
		}
		GL.End();
		GL.PopMatrix();
	...
	public void drawImagaByDrawTexture(Image image, float x, float y)
	{
		if (image != null)
		{
			GUI.DrawTexture(new Rect(x + (float)translateX, y + (float)translateY, image.getWidth(), image.getHeight()), image.texture);
		}
	}
