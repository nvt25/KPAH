...
public class WindowInfoScr : MyScreen
{
	...
	{
		return (me != null) ? me : (me = new WindowInfoScr());
	}
	...
	private void setLim()
	{
		if (!is29)
		{
			...
			if (index != null)
			{
				...
				{
					if (Char.inventory != null && listTemp != null)
					{
						int num2 = Char.inventory.size() + listTemp.size();
				...
				{
					int num4 = Char.inventory.size() + MainChar.listPotion.Length - 2;
	...
	private void setLeftKeepAndSellItem()
	{
		ActionPerform action = delegate
		{
			...
			if (selected % numW < 3)
			{
				...
				{
					item = (ItemInInventory)Char.inventory.elementAt(num);
				...
				else if (num < Char.inventory.size() + MainChar.gemItem.size() && index[focusTab] == 23)
				{
					GemTemplate gemTemplate = (GemTemplate)MainChar.gemItem.elementAt(num - Char.inventory.size());
	...
	private void setSelectedKeepAndSellItem()
	{
		ActionPerform action = delegate
		{
			...
			if (selected % numW < 3)
			{
				if (num < Char.inventory.size())
				{
					itemInInventory = (ItemInInventory)Char.inventory.elementAt(num);
				...
				else if (num < Char.inventory.size() + MainChar.gemItem.size())
				{
					GemTemplate gemTemplate = (GemTemplate)MainChar.gemItem.elementAt(num - Char.inventory.size());
					if (gemTemplate != null)
					{
	...
	public void setSelectedImbue()
	{
		...
		if ((index == 4 && (idImbue == -1 || xx >= Char.inventory.size())) || (index != 4 && ((xx == 1 && idImbue != -1) || (xx != 1 && num < MainChar.gemitemImbue.size()))))
		{
			...
			ActionPerform actionPerform = command.action = delegate
			{
				if (index == 4)
				{
					if (xx < Char.inventory.size())
					{
						ItemInInventory itemInInventory = (ItemInInventory)Char.inventory.elementAt(xx);
					...
					else
					{
						try
						{
							...
							int num2 = xx - Char.inventory.size();
				...
				else if (xx == 1)
				{
					if (idImbue != -1)
					{
						ItemInInventory itemInInventory2 = (ItemInInventory)Char.inventory.elementAt(idImbue);
				...
				numW = Char.inventory.size() + listTemp.size();
				if (numW < 6)
				{
					numW = 6;
				}
	...
	public void doShowItemInfoImbue()
	{
		...
		if (num == 4)
		{
			if (num2 < Char.inventory.size())
			{
				ItemInInventory item = (ItemInInventory)Char.inventory.elementAt(num2);
				showItemInventoryInfo(item, isSell, num2 * wSmall, 4 * wSmall);
				return;
			}
			for (int i = 0; i < listTemp.size(); i++)
			{
				if (i == num2 - Char.inventory.size())
				{
					GemTemplate gemTemplate = (GemTemplate)listTemp.elementAt(i);
					GemTemp gemByID = Res.getGemByID(gemTemplate.id);
		...
		{
			ItemInInventory itemInInventory = (ItemInInventory)Char.inventory.elementAt(idImbue);
	...
	{
		mVector mVector = new mVector();
		if (selected < Char.inventory.size())
		{
			bool flag = false;
			ItemInInventory item = (ItemInInventory)Char.inventory.elementAt(selected);
		...
		{
			int num = selected - Char.inventory.size();
			int num2 = 0;
	...
	private void updateKeyMain()
	{
		...
		bool flag = false;
		if (index[focusTab] == 28)
		{
			xBegin = -10 + xTr;
			yBegin = 5 * newZise + 10 + yTr;
			wTouch = 6 * newZise + 10;
			hTouch = 60;
			curentTabUpdate = index[focusTab];
			if (Canvas.isPointer(-10 + xTr, newZise + yTr, 6 * newZise + 10, 3 * newZise) && Canvas.isPointerClick)
			{
				isPaintFc = false;
				isSelectCheDo = true;
				xCus = -20;
				...
				if (Math.abs(Canvas.pyLast - Canvas.py) < 10 && Math.abs(Canvas.pxLast - Canvas.px) < 10)
				{
					selected = 0;
					numW = 6;
					int num = (Canvas.py - 89 - yTr) / 34;
					int num2 = (Canvas.px - 20 - xTr) / 34;
					selected = num * numW + num2;
					if (selected < 0)
					{
						selected = 0;
					}
		...
		else if (index[focusTab] == 21 || index[focusTab] == 25)
		{
			curentTabUpdate = index[focusTab];
			xBegin = -10 + xTr;
			yBegin = 5 * newZise + 10 + yTr;
			wTouch = 5 * newZise + 10;
			int num3 = Char.inventory.size() + listTemp.size();
			cmxLim = num3 * newZise - w + 6 - 60;
			if (cmxLim < 0)
			{
				cmxLim = 0;
			}
			hTouch = 60;
			if (Canvas.isPointer(-10 + xTr, newZise + yTr, 6 * newZise + 10, 4 * newZise + 30) && Canvas.isPointerClick)
			{
				isPaintFc = false;
				numW = 3;
				int num4 = 25;
				isSelectCheDo = true;
				int num5 = (Canvas.py - newZise - 10 - yTr) / newZise;
				if (num5 < 0)
				{
					num5 = 0;
				}
				...
				if (Canvas.px <= num4 + newZise + xTr && Canvas.px >= num4 + xTr)
				{
					selected = num5 * 3;
				}
				else if (Canvas.px >= num4 + 4 * newZise - 20 + xTr && Canvas.px <= num4 + 6 * newZise + xTr)
				{
					selected = num5 * 3 + 2;
				}
				else
		...
		else if (index[focusTab] == 26 || index[focusTab] == 32)
		{
			...
			if (index[focusTab] == 32)
			{
				setCamEpDoThu();
				xBegin = xTr + 25;
				yBegin = posEpNgoc[3][1] + yTr + 103;
				wTouch = 7 * wCell + 10;
				hTouch = 70;
			}
			curentTabUpdate = index[focusTab];
			int num6 = listTemp.size();
			cmxLim = num6 * 42 - wTouch + wCell;
			if (index[focusTab] == 32)
			{
				num6 = listItemAnimal.size() + listTemp.size();
				cmxLim = num6 * 34 - wTouch + 25;
			}
			if (cmxLim < 0)
			{
				cmxLim = 0;
		...
		else if (index[focusTab] == 7)
		{
			if (Canvas.currentDialog == null && Canvas.isPointer(15 + xTr, 55 + yTr, 206, 196))
			{
				if (Canvas.isPointerClick && !Canvas.menu.showMenu)
				{
					int num7 = (Canvas.py - 65 - yTr) / (newZise + 10);
					int num8 = (Canvas.px - 14 - xTr) / newZise;
					isPaintfc = false;
					if (selected == num7 * numW + num8)
					{
						if (Math.abs(Canvas.pyLast - Canvas.py) < 10 && Math.abs(Canvas.pxLast - Canvas.px) < 10)
						{
							if (center != null)
					...
					if (newSkill != null)
					{
						string[] array = new string[newSkill.size()];
						for (int i = 0; i < newSkill.size(); i++)
						{
							string text = (string)newSkill.elementAt(i);
				...
				{
					if (Math.abs(Canvas.px - Canvas.pxLast) >= 5 || Math.abs(Canvas.py - Canvas.pyLast) >= 5)
					{
						isPaintfc = true;
					}
					else
		...
		{
			if (Canvas.isPointer(25 + xTr, 57 + yTr, 5 * (wCell + 12), hView - (newZise + 53)))
			{
				if (Canvas.isPointerDown)
				{
				...
				if (Canvas.isPointerClick)
				{
					...
					if (Math.abs(Canvas.pyLast - Canvas.py) < 10 && Math.abs(Canvas.pxLast - Canvas.px) < 10)
					{
						int num9 = (cmtoY + Canvas.py - 57 - yTr) / 34;
						int num10 = (Canvas.px - 20 - xTr) / 34;
						selectedGD = num9 * numW + num10;
						if (selectedGD <= 0)
						{
							selectedGD = 0;
						}
			...
			else if (Canvas.isPointer(25 + xTr, 65 + hView - (newZise + 53) + yTr, 5 * (wCell + 12), newZise))
			{
				if (Canvas.isPointerDown)
				{
					selectedGD = -1;
					...
					int num11 = checktotalItemTrade();
					cmxLim = num11 * smallS - (wKhung - 6 * newZise);
					cmtoX = pa + (Canvas.pxLast - Canvas.px);
					if (cmtoX < -10 - Canvas.pxLast)
					{
						cmtoX = -10 - Canvas.pxLast;
					}
					if (cmtoX > cmxLim + Canvas.pxLast)
					{
						cmtoX = cmxLim + Canvas.pxLast;
					}
				}
				else if (Canvas.isPointerClick)
				{
					...
					isPaintfc = false;
					trans = false;
					int num12 = selected = (cmtoX + Canvas.px - 25 - xTr) / 34;
					int num13 = MainChar.listPotion.Length + Char.inventory.size();
					if (selected > num13 - 2)
					{
						selected = num13 - 2;
					}
					if (Math.abs(Canvas.pyLast - Canvas.py) < 10 && Math.abs(Canvas.pxLast - Canvas.px) < 10 && center != null)
					{
						idTrade = 1;
						center.perform();
					}
		...
		else if (index[focusTab] == 22 || index[focusTab] == 23)
		{
			if (Canvas.isPointer(25 + xTr, 55 + yTr, 5 * (wCell + 12), hView - 50))
			{
				if (Canvas.isPointerDown)
				{
					if (!Canvas.isMove && !Canvas.isPointerClick)
					...
					cmtoY = pb + (Canvas.pyLast - Canvas.py);
					if (cmtoY < -Canvas.pyLast)
					{
						cmtoY = -Canvas.pyLast;
					}
					if (cmtoY > cmyLim + Canvas.pyLast)
					{
						cmtoY = cmyLim + Canvas.pyLast;
					}
				}
				if (Canvas.isPointerClick)
				{
					...
					if (Math.abs(Canvas.pyLast - Canvas.py) < 10 && Math.abs(Canvas.pxLast - Canvas.px) < 10)
					{
						int num14 = (cmtoY + Canvas.py - 55 - yTr) / 34;
						int num15 = (Canvas.px - 20 - xTr) / 34;
						setCmdCenter();
						selected = num14 * numW + num15;
						if (selected < 0)
						{
							selected = 0;
						}
		...
		{
			int num16 = (index[focusTab] == 0) ? 73 : 53;
			curentTabUpdate = index[focusTab];
			xBegin = 15 + xTr;
			yBegin = 55 + yTr;
		...
		else if (Canvas.isPointer(x + x0 - 7 + xTr, y + y0 - 10 + yTr, w + 20, h + 20))
		{
			...
			if (Canvas.isPointerDown && index[focusTab] != 1)
			{
				if (!isShowText)
				{
					if (!trans)
					{
						...
						pa = cmx;
						trans = true;
						int num17 = (cmtoY + Canvas.py - (y + y0) - yTr) / hSmall;
						int num18 = (cmtoX + Canvas.px - (x + x0) - xTr) / wSmall;
						setCmdCenter();
						if (index[focusTab] != 3 && index[focusTab] != 32)
						{
							selected = num17 * numW + num18;
						}
					}
				}
				else
			...
			if (Canvas.isPointerClick)
			{
				...
				Canvas.isPointerClick = false;
				trans = false;
				if (Math.abs(i2) < 10 && Math.abs(i3) < 10 && index[focusTab] != 3)
				{
					if (index[focusTab] != 32)
					{
						setCmdCenter();
					...
					else if (Canvas.py >= posEpNgoc[1][1] + 70 + y + y0 + yTr && Math.abs(Canvas.pxLast - Canvas.px) <= 10)
					{
						int num19 = (cmtoY + Canvas.py - (y + y0) - yTr) / hSmall;
						int num20 = (cmtoX + Canvas.px - (x + x0) - xTr) / wSmall;
						selected = (num19 - 1) * numW + num20;
						if (center != null)
						{
							center.perform();
						}
	...
	public bool setSizeKeepItem(int i)
	{
		...
		if (Char.inventory.size() + MainChar.gemItem.size() < num3)
		{
			numH = num3 / 3 + 1;
		}
		else
		{
			numH = (Char.inventory.size() + MainChar.gemItem.size()) / 3 + 1;
		}
		if (numH < 5)
		{
			numH = 5;
		...
		if ((selected % numW == 2 && i == 1) || (selected % numW == 3 && i == -1))
		{
			if (i == -1)
			{
				cmyLim = ((((Char.inventory.size() + MainChar.gemItem.size()) / 3 + 1) * wSmall - index[focusTab] != 22) ? (h + num) : h);
			}
			else
			{
	...
	public override void paint(MyGraphics g)
	{
		try
		{
			...
			if (index[focusTab] != 27 && index[focusTab] != 29 && index[focusTab] != 0)
			{
				...
				int width = MFont.normalFont[0].getWidth(name);
				if (width > 88)
				{
					transX -= dirX;
				...
				MFont.normalFont[0].drawString(g, name, 66 + transX, -3, MFont.CENTER);
				if (cmxname != 0)
				{
					MFont.normalFont[0].drawString(g, firName, 66 + 100 * dir, -3, MFont.CENTER);
				}
			}
			g.translate(cmxname, 0);
			g.translate(x0, y0);
			...
			case 1:
			{
				isDefault = false;
				Canvas.resetTrans(g);
				paintBG(g, 10 + xTr, 10 + yTr, wKhung, hKhung);
				g.translate(25, 65);
				int num20 = hView - 28;
				int num21 = wKhung;
				g.setColor(16774720);
				g.fillRect(Res.wTrangbi + 30 + xTr, -16 + yTr, num21 - (Res.wTrangbi + 30 + x0 - 3), num20 + 2);
				num20--;
				g.setColor(25695);
				g.fillRect(Res.wTrangbi + 31 + xTr, -15 + yTr, num21 - (Res.wTrangbi + 30 + x0 - 3) - 2, num20 + 1);
				paintCharWearing(g);
				Canvas.resetTrans(g);
				MFont.bigFont.drawString(g, "Trang bị", Canvas.hw, 18 + yTr, MFont.CENTER);
				break;
			}
			case 2:
				isDefault = false;
			...
			case 8:
			{
				...
				sbyte b3 = 24;
				g.setColor(25695);
				g.fillRect(num17 + xTr, -11 + yTr, wKhung - (x0 - 2 + 7 * wCell + b3), num16);
				num16--;
				g.setColor(16774720);
				g.drawRect(num17 + xTr, -11 + yTr, wKhung - (x0 - 2 + 7 * wCell + b3) - 1, num16);
				if (index[focusTab] == 7)
				{
					paintNewSkillLearn(g);
					Canvas.resetTrans(g);
					MFont.bigFont.drawString(g, "Học kỹ năng", Canvas.hw, 18 + yTr, MFont.CENTER);
				}
				else
				{
					Canvas.resetTrans(g);
					...
					paintTrade(g);
					Canvas.resetTrans(g);
					MFont.bigFont.drawString(g, "Giao dịch", Canvas.hw, 18 + yTr, MFont.CENTER);
				}
				break;
			}
			case 21:
			case 25:
			{
				...
				sbyte b4 = 24;
				g.setColor(25695);
				g.fillRect(num19 + xTr, -11 + yTr, wKhung - (x0 - 2 + 7 * wCell + b4), num18);
				num18--;
				g.setColor(16774720);
				g.drawRect(num19 + xTr, -11 + yTr, wKhung - (x0 - 2 + 7 * wCell + b4) - 1, num18);
				paintImbue(g);
				Canvas.resetTrans(g);
				MFont.bigFont.drawString(g, name, Canvas.hw, 18 + yTr, MFont.CENTER);
				break;
			}
			case 22:
			case 23:
			{
				...
				sbyte b7 = 24;
				g.setColor(25695);
				g.fillRect(num27 + xTr, -11 + yTr, wKhung - (x0 - 2 + 7 * wCell + b7), num26);
				num26--;
				g.setColor(16774720);
				g.drawRect(num27 + xTr, -11 + yTr, wKhung - (x0 - 2 + 7 * wCell + b7) - 1, num26);
				Canvas.resetTrans(g);
				g.translate(20, 55);
				paintKeepItem(g);
				Canvas.resetTrans(g);
				if (index[focusTab] == 22)
				{
					MFont.bigFont.drawString(g, "Kho đồ", Canvas.hw, 18 + yTr, MFont.CENTER);
				}
				else
				{
					MFont.bigFont.drawString(g, "Bán đồ", Canvas.hw, 18 + yTr, MFont.CENTER);
				}
				break;
			}
			case 26:
			{
				...
				sbyte b6 = 24;
				g.setColor(25695);
				g.fillRect(num25 + xTr, -11 + yTr, wKhung - (x0 - 2 + 7 * wCell + b6), num24);
				num24--;
				g.setColor(16774720);
				g.drawRect(num25 + xTr, -11 + yTr, wKhung - (x0 - 2 + 7 * wCell + b6) - 1, num24);
				paintEpNgoc(g);
				Canvas.resetTrans(g);
				MFont.bigFont.drawString(g, name, Canvas.hw, 18 + yTr, MFont.CENTER);
				break;
			...
			case 28:
			{
				...
				sbyte b5 = 24;
				g.setColor(25695);
				g.fillRect(num23 + xTr, -11 + yTr, wKhung - (x0 - 2 + 7 * wCell + b5), num22);
				num22--;
				g.setColor(16774720);
				g.drawRect(num23 + xTr, -11 + yTr, wKhung - (x0 - 2 + 7 * wCell + b5) - 1, num22);
				paintCheDo(g);
				Canvas.resetTrans(g);
				MFont.bigFont.drawString(g, name, Canvas.hw, 18 + yTr, MFont.CENTER);
				break;
			...
			{
				isDefault = false;
				g.setClip(0, 0, numW * wSmall + 1, 5 * hSmall + 1);
				g.translate(0, -cmy);
				int num14 = cmy / hSmall;
				if (num14 < 0)
				{
					num14 = 0;
				}
				int num15 = num14 + w / hSmall;
				for (int m = num14; m < num15; m++)
				{
					for (int n = 0; n < numW; n++)
					{
						paintGrid(g, n * wSmall, m * hSmall);
					}
				}
				if (!isChangeTab)
				{
					g.setColor(10595790);
					g.fillRect(selected % numW * wSmall + 1, selected / numW * hSmall + 1, wSmall - 2, hSmall - 2);
				}
				paintProduct(g);
				break;
			}
			case 31:
			{
				...
				int num13 = wKhung;
				g.setColor(16774720);
				g.fillRect(Res.wTrangbi + 30 + xTr, -16 + yTr, num13 - (Res.wTrangbi + 30 + x0 - 3), num12 + 2);
				num12--;
				g.setColor(25695);
				g.fillRect(Res.wTrangbi + 31 + xTr, -15 + yTr, num13 - (Res.wTrangbi + 30 + x0 - 3) - 2, num12 + 1);
				paintAnimalWearing(g);
				Canvas.resetTrans(g);
				MFont.bigFont.drawString(g, "Trang bị thú", Canvas.hw, 18 + yTr, MFont.CENTER);
				break;
			}
			case 32:
			{
				...
				sbyte b2 = 24;
				g.setColor(25695);
				g.fillRect(num11 + xTr, -11 + yTr, wKhung - (x0 - 2 + 7 * wCell + b2), num10);
				num10--;
				g.setColor(16774720);
				g.drawRect(num11 + xTr, -11 + yTr, wKhung - (x0 - 2 + 7 * wCell + b2) - 1, num10);
				paintEpDothu(g);
				Canvas.resetTrans(g);
				MFont.bigFont.drawString(g, name, Canvas.hw, 18 + yTr, MFont.CENTER);
				break;
			...
			default:
				try
				{
					...
					{
						int num6 = num2 - 130;
						MFont.smallFont.drawString(g, countTabMyitem + 1 + "/" + Canvas.gameScr.mainChar.limItemBag, 104 + xTr, num6 - 4 + yTr, 2);
						g.drawImage(Res.imgBar, 4 + xTr, num6 + yTr, 3);
						g.drawRegion(Res.imgBar, 0, 0, 11, 7, 2, num4 - wCell / 2 + xTr, num6 + yTr, 3);
					}
					...
					default:
						if (index[focusTab] != 24)
						{
							...
							MFont.bigFont.drawString(g, "Gian hàng", num / 4 + 10 + xTr, 18 + yTr, MFont.CENTER);
							MFont.bigFont.drawString(g, "Hành trang", num / 2 + num / 4 + xTr, 18 + yTr, MFont.CENTER);
							g.setColor(10591392);
	...
	private void paintKeepAndSellItem(MyGraphics g)
	{
		...
		for (int k = 0; k < Char.inventory.size(); k++)
		{
			num = k % 3 * 18;
			num2 = k / 3 * 18;
			ItemInInventory itemInInventory = (ItemInInventory)Char.inventory.elementAt(k);
			if (itemInInventory.isSelling)
			{
				g.setColor(7706352);
	...
	private void paintEpNgoc(MyGraphics g)
	{
		...
		int num = 0;
		paintGemItemHopThanh(g, num, 164);
		num += listTemp.size();
	}
	...
	private void paintGemItemCheDo(MyGraphics g, int count, int y)
	{
		...
		for (int i = 0; i < num; i++)
		{
			...
			{
				g.setColor(3276851, 0.5f);
				g.fillRect(count * 42 - 21, y, 42, 42);
			}
			g.setColor(10595790, 0.5f);
			...
			g.drawImage(Res.imgCell[1], count * 42 - 21, y, 0);
			GemTemplate gemTemplate = (GemTemplate)listTemp.elementAt(i);
	...
	private void paintGemItemHopThanh(MyGraphics g, int count, int y)
	{
		for (int i = 0; i < listTemp.size(); i++)
		{
			if (selected == i && isPaintFc)
			{
				g.fillRect(count * 42, y, 41, 42);
			}
			g.drawImage(Res.imgCell[1], count * 42, y, 0);
			GemTemplate gemTemplate = (GemTemplate)listTemp.elementAt(i);
			Res.paintGem(g, Res.getGemByID(gemTemplate.id).idImage, count * 42 + 21, y + 17);
			GemTemp gemByID = Res.getGemByID(gemTemplate.id);
			string s = gemByID.name.Substring(gemByID.name.Length - 1);
			try
			{
				int num = int.Parse(s);
				MFont.smallFont.drawString(g, num + string.Empty, count * 42 + 42, y + 2, MFont.RIGHT);
			}
			catch (Exception)
			{
			}
			MFont.smallFont.drawString(g, gemTemplate.number + string.Empty, count * 42 + 42, y + 28, MFont.RIGHT);
			count++;
		}
	}
	...
	private void paintGemItem(MyGraphics g, int count, int y)
	{
		for (int i = 0; i < listTemp.size(); i++)
		{
			...
			{
				g.setColor(3276851, 0.5f);
				g.fillRect(count * 42 - 21 + xTr, y + yTr, 42, 42);
			}
			g.drawImage(Res.imgCell[1], count * 42 - 21 + xTr, y + yTr, 0);
			GemTemplate gemTemplate = (GemTemplate)listTemp.elementAt(i);
			if (gemTemplate != null)
			{
				Res.paintGem(g, Res.getGemByID(gemTemplate.id).idImage, count * 42 + xTr, y + 17 + yTr);
				if (index[focusTab] != 21)
				{
					GemTemp gemByID = Res.getGemByID(gemTemplate.id);
					string s = gemByID.name.Substring(gemByID.name.Length - 1);
					...
					{
						int num = int.Parse(s);
						MFont.smallFont.drawString(g, num + string.Empty, count * 42 + 5 + xTr, y + 2 + yTr, MFont.LEFT);
					}
					catch (Exception)
					{
					}
				}
				MFont.smallFont.drawString(g, gemTemplate.number + string.Empty, count * 42 + 21 + xTr, y + 22 + yTr, MFont.RIGHT);
			}
			count++;
		}
	}
	...
	private void paintImbue(MyGraphics g)
	{
		...
		for (int i = 0; i < MainChar.gemitemImbue.size(); i++)
		{
			...
			Res.paintGem(g, Res.getGemByID(realID.ID).idImage, xTr + i % 2 * (5 * wCell) + wCell / 2 + 6, yTr + i / 2 * 42 + hSmall / 2 - 4);
		}
		g.setClip(-10 + xTr, 4 * newZise - 4 + yTr, 5 * newZise + 20, 52);
		...
		if (isPaintFc)
		{
			g.fillRect(idfcsetSelectedImbue * 42 - 21 + xTr, 164 + yTr, 42, 42);
		}
		for (int j = 0; j < Char.inventory.size(); j++)
		{
			ItemInInventory itemInInventory = (ItemInInventory)Char.inventory.elementAt(j);
			g.drawImage(Res.imgCell[1], num * 42 - 21 + xTr, 164 + yTr, 0);
			itemInInventory.paint(g, num * 42 + xTr, 185 + yTr);
			num++;
		}
		paintGemItem(g, num, 164);
		num += listTemp.size();
	}
	...
	private void paintDapBG(MyGraphics g, int x, int y)
	{
		if (!isChangeTab && Canvas.gameTick % 10 > 3 && selected / numW < 4 && isSelectCheDo)
		{
			...
			{
				g.fillRect(63 + xTr, -10 + yTr, 84, 168);
			}
			else
			{
				int num = selected % numW;
		...
		for (int i = 0; i < 8; i++)
		{
			g.drawImage(Res.imgCell[1], i / 4 * (5 * wCell) + xTr, i % 4 * 42 - 10 + yTr, 0);
		}
		g.setColor(10595790);
	...
	private void paintTrade(MyGraphics g)
	{
		...
		g.setColor(10595790);
		for (int m = 0; m < Char.inventory.size(); m++)
		{
			ItemInInventory itemInInventory = (ItemInInventory)Char.inventory.elementAt(m);
			if (itemInInventory == null)
			{
				continue;
			}
	...
	private void paintCharWearing(MyGraphics g)
	{
		...
		for (int i = 0; i < 5; i++)
		{
			g.fillRect(-3 + xTr, -10 + i * 42 + yTr, 40, 40);
			g.fillRect(147 + xTr, -10 + i * 42 + yTr, 40, 40);
		}
		g.setColor(4802889);
		...
		for (int j = 0; j < 5; j++)
		{
			g.fillRect(-2 + xTr, -9 + j * 42 + yTr, 38, 38);
			g.fillRect(148 + xTr, -9 + j * 42 + yTr, 38, 38);
		}
		if (Canvas.gameTick % 10 > 3 && isPaintFc)
		{
			...
			if (selected % 3 != 1)
			{
				...
				g.fillRect(-2 + num * 150 + xTr, selected / numW * 42 - 9 + yTr, 38, 38);
			}
			else if (selected != 4)
			{
				g.fillRect(111 + xTr, -10 + yTr + 176, 28, 28);
	...
	private void paintAnimalWearing(MyGraphics g)
	{
		...
		if (Canvas.gameTick % 10 > 3 && isPaintFc)
		{
			...
			if (selected % 2 == 0)
			{
				g.fillRect(-2 + xTr, selected / 2 * 42 - 9 + yTr, 38, 38);
			}
			else
			{
				g.fillRect(148 + xTr, selected / 2 * 42 - 9 + yTr, 38, 38);
			}
		}
		int num = 0;
	...
	private void paintProduct(MyGraphics g)
	{
		if (isDefault)
		{
			...
			for (int i = num; i < num2; i++)
			{
				...
				command = (Command)list.elementAt(i);
				int num3 = i % 5 * (wSmall + 10) + (wSmall + 10) / 2 - 7;
				int num4 = i / 5 * (hSmall + 10) + (hSmall + 10) / 2 - 10;
				command.paint(g, num3, num4);
			}
			return;
		}
		int num5 = cmy / hSmall * numW;
		if (num5 < 0)
		{
			num5 = 0;
		}
		int num6 = num5 + numW * numH;
		if (num6 > list.size())
		{
			num6 = list.size();
		}
		...
		for (int j = num5; j < num6; j++)
		{
			...
			int num7 = j % numW * wSmall + wSmall / 2;
			int num8 = j / numW * hSmall + hSmall / 2;
			command2.paint(g, num7, num8);
		}
	}
		...
		for (int i = 0; i < Res.shopTemplate.size(); i++)
		{
			...
			if (shop.shopType == focusTab && shop.isSell)
			{
				...
				ActionPerform action = delegate
				{
					...
					setShowTextNew(empty, ii % numW * 18, ii / numW * 18);
				};
				ActionPaintCmd actionPaint = delegate(MyGraphics g, int x, int y)
				{
					GameData.paintIcon(g, (short)(Res.getShopByID(shop.id).idImage + 5500), x, y);
					MFont.smallFont.drawString(g, (short)((shop.id <= 1) ? 1 : Res.getShopByID(shop.id).value) + string.Empty, x + 8, y + 10, MFont.RIGHT);
				};
				command.action = action;
				command.actionPaint = actionPaint;
		...
		try
		{
			for (int i = 1; i < MainChar.listPotion.Length; i++)
			{
				...
				if (getCountPotion(i) > 0)
				{
					...
					ActionPerform action = delegate
					{
						setShowTextNew(getPotionInfo(aa), ii % numW * 18, ii / numW * 18);
					};
					ActionPaintCmd actionPaint = delegate(MyGraphics g, int x, int y)
					{
			...
			{
				int aa5 = num;
				GemTemplate item5 = (GemTemplate)MainChar.gemItemKhoa.elementAt(k);
				GemTemp ge2 = Res.getGemByID(item5.id);
				Command command3 = new Command();
				command3.action = delegate
				{
					showGemItemKhoaInfo(item5.id, aa5 % numW * 18, aa5 / numW * 18);
				};
				command3.actionPaint = delegate(MyGraphics g, int x, int y)
				{
					if (Res.getGemByID(item5.id) != null)
					{
						if (aa5 - ((index[focusTab] == 0) ? (countTabMyitem * 42) : 0) != selected)
						{
							g.setColor(3276851, 0.5f);
							g.fillRect(x - 20, y - 20, 40, 40);
						}
						Res.paintGemKhoa(g, Res.getGemByID(item5.id).idImage, x, y);
						if (ge2.isEff != -1 && ge2.isEff >= 0 && ge2.isEff < GemTemp.color.Length)
						{
						...
						MFont.smallFont.drawString(g, item5.number + string.Empty, x + 8, y + 10, MFont.RIGHT);
					}
				};
				mVector.addElement(command3);
				num++;
			...
			{
				int aa4 = num;
				GemTemplate item4 = (GemTemplate)MainChar.gemItem.elementAt(l);
				GemTemp ge = Res.getGemByID(item4.id);
				Command command4 = new Command();
				ActionPerform action3 = delegate
				{
					showGemItemInfo(item4.id, aa4 % numW * 18, aa4 / numW * 18);
				};
				ActionPaintCmd actionPaint3 = delegate(MyGraphics g, int x, int y)
				{
					if (Res.getGemByID(item4.id) != null)
					{
						Res.paintGem(g, Res.getGemByID(item4.id).idImage, x, y);
						...
						MFont.smallFont.drawString(g, item4.number + string.Empty, x + 11, y + 10, MFont.RIGHT);
					}
				};
			...
			{
				int ii4 = num;
				AnimalInfo item3 = (AnimalInfo)Char.animal.elementAt(m);
				Command command5 = new Command();
				ActionPerform action4 = delegate
				{
					if (!item3.info.Equals(string.Empty))
					{
						if (item3.typeAnimal == 0)
						{
							setShowText(ItemInInventory.setTemp(item3.name + " lv" + item3.level, "0") + ItemInInventory.setTemp(item3.info, "0"), ii4 % numW * 18, ii4 / numW * 18);
						}
						else
						{
							long num4 = item3.totalTimeCon - (mSystem.getCurrentTimeMillis() - item3.timeStart) / 60000;
							setShowText(ItemInInventory.setTemp(item3.name, "0") + ItemInInventory.setTemp(item3.info, "0") + ItemInInventory.setTemp("Còn lại: " + num4 + " phút", "0"), ii4 % numW * 18, ii4 / numW * 18);
						}
					}
				};
			...
			{
				int ii3 = num;
				ItemInInventory item2 = (ItemInInventory)Char.inventory.elementAt(n);
				Command command6 = new Command();
				ActionPerform action5 = delegate
				{
					showItemInventoryInfo(item2, isSell, ii3 % numW * 18, ii3 / numW * 18);
				};
			...
			for (int num3 = 0; num3 < num2; num3++)
			{
				...
				if (MainChar.itemQuest[num3] > 0)
				{
					...
					{
						setShowTextNew(getInfoItemQuest(aa3), ii2 % numW * 18, ii2 / numW * 18);
					};
	...
	private string getPotionInfo(int aa)
	{
		...
		if (aa < 7 || aa >= 14)
		{
			arg = "0" + MainChar.listPotion[aa].name;
		...
		{
			arg = Res.PORTION_ITEM_NAME_CLAN;
			arg = arg + "\nSố lượng: " + Canvas.gameScr.mainChar.potions[aa];
		}
		return arg;
	}
	...
	private void showMenuForPotion(int potionType)
	{
		...
		if (potionType < 14 || potionType >= 21)
		{
			...
			ActionPerform actionPerform3 = command3.action = delegate
			{
				...
				{
					int ii = i;
					Command command5 = new Command("Phím số " + (7 + 2 * i));
	...
	private void dosetLeftSellGemItem()
	{
		...
		ActionPerform action = delegate
		{
			WindowInfoScr windowInfoScr = this;
			if (!setMaxxreadyBuy())
			{
					...
					ActionPerform ok = delegate
					{
						try
						{
							...
							if (number >= 0)
							{
								...
								{
									windowInfoScr.setShowText("Hết tiền", windowInfoScr.selected % windowInfoScr.numW * 18, windowInfoScr.selected / windowInfoScr.numW * 18);
								}
								else
								{
									ActionPerform yesAction = delegate
									{
										int num2 = 0;
										for (int i = 0; i < windowInfoScr.readyBuy.size(); i++)
										{
											GemTemp gemTemp = (GemTemp)windowInfoScr.readyBuy.elementAt(i);
										...
										if (num2 == 0)
										{
											...
											windowInfoScr.readyBuy.addElement(o);
										}
										if (g.typeMoney == 0)
										{
	...
	private void doSetLeftPotion()
	{
		ActionPerform action = delegate
		{
			...
			if (!setMaxxreadyBuy())
			{
				ItemInInventory item = (ItemInInventory)Canvas.gameScr.shop.elementAt(selected);
				int num = 0;
				int num2 = -1;
				if (readyBuy.size() > 0)
				{
					...
					{
						ItemInInventory itemInInventory = (ItemInInventory)readyBuy.elementAt(i);
						if (itemInInventory.potionType == item.potionType)
						{
							num = item.number;
							num2 = i;
							break;
				...
				if (num + Canvas.gameScr.mainChar.potions[item.potionType] >= MAX_POTION)
				{
					setShowText("MAX: " + MAX_POTION, selected % numW * 18, selected / numW * 18);
				}
				else
				{
					int price = Res.potionTemplates[item.potionType].price;
					if (price > Canvas.gameScr.mainChar.charXu)
					{
					...
					else
					{
						...
						ActionPerform ok = delegate
						{
							try
							{
								...
								if (num3 < 0)
								{
									Canvas.startOKDlg("Không được nhập số âm.");
								}
								else if (num3 > 120)
								{
									Canvas.startOKDlg("Chỉ được mua nhiều nhất 120 đơn vị một lần.");
								}
								else
								{
									ItemInInventory itemInInventory2 = (ItemInInventory)windowInfoScr.readyBuy.elementAt(bb);
									itemInInventory2.number += (short)num3;
									Canvas.gameScr.mainChar.charXu -= price;
									Canvas.endDlg();
									windowInfoScr.countPop = 0;
									windowInfoScr.closeText();
									if (windowInfoScr.isDefault)
									{
										windowInfoScr.setShowTextNew(item.getTemplateDescription(), windowInfoScr.selected % windowInfoScr.numW * 18, windowInfoScr.selected / windowInfoScr.numW * 18);
									}
									else
									{
										windowInfoScr.setShowText(item.getTemplateDescription(), windowInfoScr.selected % windowInfoScr.numW * 18, windowInfoScr.selected / windowInfoScr.numW * 18);
									}
								}
							}
							catch (Exception)
							{
								Canvas.startOKDlg("Không được nhập chữ");
							}
						};
						Canvas.inputDlg.setInfo("Số lượng: ", ok, TField.INPUT_TYPE_NUMERIC, 10, isMaxW: true);
						Canvas.inputDlg.show();
					}
				}
			}
	...
	public void updateKeyDong()
	{
		...
		if (timeOpen > 0)
		{
			...
			case 1:
				...
				{
					int num5 = (Canvas.py - yBegin) / 40;
					int num6 = (Canvas.px - xBegin) / wSmall;
					int num7 = num5 * numW + num6;
					if (num7 < 0)
					{
						num7 = 0;
					}
				...
				{
					if (Canvas.px <= x0 + 150 + xTr || Canvas.px > x0 + 190 + xTr || Canvas.py <= 50 + yTr || Canvas.py > 280 + yTr)
					{
						break;
					}
					int num8 = (Canvas.py - yBegin) / 40;
					int num9 = (Canvas.px - xBegin) / wSmall;
					int num10 = num8 * numW + num9 + 1;
					if (num10 < 2)
					{
						num10 = 2;
					}
			...
			case 25:
			{
				int num3 = (Canvas.py - 65 - yTr) / newZise;
				int num4 = (cmtoX + Canvas.px - 5 - xTr) / newZise;
				if (num3 == 5)
				{
					num3 = 4;
				}
				if (num3 < 4)
				{
					numW = 3;
					wSmall = newZise;
					num4 = (Canvas.px - newZise - xTr) / newZise;
				}
				else if (num3 == 4)
				{
					isSelectCheDo = false;
				...
				if (num3 != 4 && num3 != 5 && (index[focusTab] == 21 || index[focusTab] == 25))
				{
					setUpdatePointerImbue(num4, num3);
				}
				else
				{
					if ((index[focusTab] == 21 || index[focusTab] == 25) && num3 == 4 && selected / numW != 4)
					{
						selected = 10;
						setSizeImbue(1);
					}
			...
			{
				isSelectCheDo = false;
				int num13 = (cmtoX + Canvas.px - xTr - 25) / 42;
				if (num13 > listTemp.size())
				{
					num13 = listTemp.size() - 1;
				}
				if (num13 < 0)
				{
					num13 = 0;
			...
			case 32:
			{
				...
				int num2 = 0;
				selected = -1;
				if (Canvas.px >= posEpNgoc[0][0] + xTr + 25 && Canvas.px <= posEpNgoc[0][0] + xTr + 217 && Canvas.py >= posEpNgoc[3][1] + yTr + 103 && Canvas.py <= posEpNgoc[3][1] + yTr + 135)
				{
					numH = -1;
					isNguyenLieu = true;
					num = (Canvas.px - 25 - xTr) / 32;
					num2 = listTemp.size();
				}
				else
				{
					numH = 1;
					isNguyenLieu = false;
					num = (cmtoX + Canvas.px - 25 - xTr) / 32;
					num2 = listItemAnimal.size() + listTemp.size();
				}
				closeText();
				if (num > num2)
				{
			...
			case 31:
				...
				else if (Canvas.px > x0 + xTr && Canvas.px <= x0 + 40 + xTr && Canvas.py > 60 + yTr && Canvas.py <= 260 + yTr)
				{
					...
					if (Math.abs(Canvas.pyLast - Canvas.py) < 10 && Math.abs(Canvas.pxLast - Canvas.px) < 10)
					{
						if (center != null)
						{
							Out.println("trang bi thu cua nguoi khac 11");
							center.perform();
						}
						setCmdCenter();
					}
				...
				{
					if (Canvas.px <= x0 + 150 + xTr || Canvas.px > x0 + 190 + xTr || Canvas.py <= 60 + yTr || Canvas.py > 260 + yTr)
					{
						break;
					}
					int num12 = (Canvas.py - yBegin) / 40;
					selected = num12 * 2 + 1;
					if (selected > 9)
					{
						selected = 9;
					...
					if (Math.abs(Canvas.pyLast - Canvas.py) < 10 && Math.abs(Canvas.pxLast - Canvas.px) < 10)
					{
						if (center != null)
						{
							Out.println("trang bi thu cua nguoi khac 2222");
							center.perform();
						}
						setCmdCenter();
					}
			...
			default:
				...
				if (countClick >= 2)
				{
					if (curentTabUpdate == 0)
					{
						doSelectedInventori();
					}
					else if (cmdShop != null && index[focusTab] != 24)
					{
						cmdShop.perform();
					}
					countClick = 0;
		...
		long num14 = count - timeDelay;
		int num15 = pyLast - Canvas.py;
		pyLast = Canvas.py;
		int num16 = pxLast - Canvas.px;
		pxLast = Canvas.px;
		if (Canvas.isPointerDown)
		{
			if (count % 2 == 0L)
		...
		transY = false;
		int num17 = (int)(count - timePointY);
		float num18 = dyTran - Canvas.py;
		float num19 = dxTran - Canvas.px;
		if (Canvas.beginMoveCmrY())
		{
			vY = num18 / (float)num17 * 10f;
		}
		int num20 = (int)(count - timePointX);
		if (Canvas.beginMoveCmrX())
		{
			vX = num19 / (float)num20 * 10f;
		}
		timePointY = -1L;
		timePointX = -1L;
	...
	public void checkSelect()
	{
		...
		int num6 = (cmtoX + Canvas.px - (num3 - 10) - xTr) / 42;
